...
import pygame
from path_search import bfs as bfs_path_search
...
def virtual_run(snake_coords, apple, path):
    ...
    __path = bfs_path_search.cal(_snake_coords[0], _snake_coords[-1], _snake_coords[:-1]) 
...
def wander(snake_coords, apple):
    ...
    while not queue.empty():
        adj = queue.get()
        path = bfs_path_search.cal(adj, snake_coords[-2], [adj] + snake_coords[:-2])
        if path:
            return adj

    return None

#calculates the next coordinate for the snake to move to. It first tries to find a path to the apple using BFS search.
def next_coord(snake_coords, apple):
    path = bfs_path_search.cal(snake_coords[0], apple, snake_coords[:-1])
...
def main():
    ...
    box_world.init("Cyber python: BFS search", 15, 13)
    box_world.draw_grid()
    box_world.update()
