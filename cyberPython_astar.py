...
import pygame
from path_search import a_star as a_path_search
...
def virtual_run(snake_coords, apple, path):
    ...
    __path = a_path_search.cal(_snake_coords[0], _snake_coords[-1], _snake_coords[:-1]) 

    return __path is not None

#the snake moves in a direction that gets it closer to the apple
def wander(snake_coords, apple):
    ...
    for adj in adjs:
        (x, y) = adj
        g = abs(a_x - x) + abs(a_y - y)

        path = a_path_search.cal(adj, snake_coords[-2], [adj] + snake_coords[:-2])
        if path:
            h = len(path)
        else: 
            h = 0
...
#calculates the next coordinate for the snake to move to. It first tries to find a path to the apple using A* search.
def next_coord(snake_coords, apple):
    path = a_path_search.cal(snake_coords[0], apple, snake_coords[:-1])
...
def main():
    ...
    box_world.init("Cyber python: A-star search", 15, 13)
    box_world.draw_grid()
    box_world.update()
